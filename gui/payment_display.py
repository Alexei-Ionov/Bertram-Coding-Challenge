...
import random
from main import display_gui_sequence
unfortunate_coworker = ""
coworkers_list = coworkers().coworkers_list
def get_least_paying_workers():
    ...
    if event.num == 1:  # Check if left mouse button was clicked
        ...
        for coworker in coworkers_list:
            if coworker.name == unfortunate_coworker:
                coworker.paid_for += payment
        root.after(5000, lambda:  display_gui_sequence(root))  # Display shop GUI after 2 seconds


def display_payment(root,payment):

    clear_gui(root)
  
    title_frame = tk.Frame(root)
    title_frame.pack(pady=20)
    least_list = get_least_paying_workers()
    str_least_list = "".join(least_list)
    ...
    button.bind("<Button-1>", lambda event, arg1=person_label, arg2=least_list, arg3=root,arg4=payment: on_button_click_make_decision(event, arg1, arg2, arg3,arg4))
